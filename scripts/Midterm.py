...
import os
import webbrowser
from correlation import cat_correlation, cat_cont_correlation_ratio
...
def create_correlation_table(correlation, correlation_method, table_title):
    ...
    for i in range(len(correlation)):
        for j in range(len(correlation.columns)):
            ...
            if variable_1 != variable_2:
                ...
                table_data.append([variable_1, variable_2, corr_value])

    df = pd.DataFrame(table_data[1:], columns=table_data[0])
    df_sorted = df.sort_values(by='Correlation', ascending=False)

    table = go.Table(
        header=dict(values=df_sorted.columns,
                    fill_color='paleturquoise',
                    align='center'),
        cells=dict(values=[df_sorted['Variable 1'], df_sorted['Variable 2'], df_sorted['Correlation']],
                   fill_color='lavender',
                   align='center'))
    ...
    # file_name = f"correlation_table_{correlation_method}.html"
...
def cont_cont_correlation(X, cont):
    ...
    fig1 = plot_correlation_matrix(correlation_matrix, "Correlation Matrix Pearson", "Variable 1", "Variable 2")

    # show  table
    if not correlation_matrix.empty:
        fig2 = create_correlation_table(correlation_matrix, " Pearson ", " Correlation Pearson Table")
...
def cat_cat_correlation(X, cat):
    ...
    df = pd.DataFrame({'Variable 1': variable_1, 'Variable 2': variable_2, 'Correlation Ratio': Correlation_ratio})

    # Pivot the dataframe to create a correlation matrix
    correlation_matrix = df.pivot(index='Variable 1', columns='Variable 2', values='Correlation Ratio')

    fig1 = plot_correlation_matrix(correlation_matrix, "Categorical-Categorical Correlation Matrix", "Variable 1",
                                   "Variable 2")
    if not correlation_matrix.empty:
        fig2 = create_correlation_table(correlation_matrix, " Tschuprow ", " Correlation Tschuprow Table")
...
def cat_cont_correlation(X, cont, cat):
    ...
    for i in range(0, len(cat)):
        for j in range(0, len(cont)):
            ...
            Correlation_ratio.append(round(cat_cont_correlation_ratio(X[cat[i]], X[cont[j]]), 5))
    ...
    fig1 = plot_correlation_matrix(correlation_matrix, "Categorical-Continuous Correlation Matrix", "Variable 1",
                                   "Variable 2")

    if not correlation_matrix.empty:
        fig2 = create_correlation_table(correlation_matrix, " Ratio Table", " Correlation Ratio Table")
        with open("my_report.html", 'a') as f:
            f.write(fig1.to_html(full_html=False, include_plotlyjs='cdn'))
            f.write(fig2.to_html(full_html=False, include_plotlyjs='cdn'))
    else:
...
def main():
    ...
    # page = urllib.urlopen('my_report.html').read()
    # print(page)
